...
function formatApacheStyle(token, req, res) {
  return token
    // common log format
    .replace('%h', formatRemoteHost(req))
    .replace('%l', '-') //we don't try to resolve this
    .replace('%u', '-') //todo: ask Slava how to find user name
    .replace('%t', formatDate(new Date()))
    .replace('%r', req.method + " " + req.originalUrl + " HTTP/" + formatHttpVersion(req))
    .replace('%>s', formatStatusCode(res))
    .replace('%b', res.headers['content-length'])
    // combined log format
    .replace('%{Referer}i', formatReferer(req))
