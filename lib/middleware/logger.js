...
module.exports = function logger(options) {
  ...
  return function logger(req, res, next) {
    ...
    if (fmt) {
      ...
      res.end = function(chunk, encoding) {
        var contentLength = checkUndef(formatContentLength(res));
...
function formatApacheStyle(token, req, res) {
  return token
    ...
    .replace('%b', checkUndef(formatContentLength(res)))
    // combined log format
...
function formatDate(date) {
    if (Date.today) {
        // requires date.js
        return date.toString("[dd/MMM/yyyy:HH:mm:ss ") + date.getUTCOffset("UTC") + "]";
    } else {
...
function formatContentLength(res) {
    return (res._headers && res._headers['content-length']) || (res.__headers && res.__headers['Content-Length']);
}
