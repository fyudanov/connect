...
function format(str, req, resp) {
  var tokens = tokenize(str);
  var res = "";
  tokens.forEach(function (token) {
    res += formatToken(token, req, resp);
  });
  return res;
}

function formatToken(token, req, resp) {
  return token
    .replace(':url', req.originalUrl)
    .replace(':method', req.method)
    .replace(':status', resp.__statusCode || resp.statusCode)
    .replace(':respponse-time', resp.respponseTime)
    ...
    .replace(/:resp\[([^\]]+)\]/g, function(_, field){
      return resp._headers
        ? (resp._headers[field.toLowerCase()] || resp.__headers[field])
        : (resp.__headers && resp.__headers[field]);
    });
}
